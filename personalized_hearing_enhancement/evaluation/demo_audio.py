...
def run_demo_audio(
    ...
    assert audiogram_tensor.shape[-1] == len(profile.frequencies), "Profile tensor and frequency grid mismatch"

    logger.info(f"Using hearing profile source: {profile_source}")
    logger.info(f"Profile device metadata: {profile.device_profile or 'N/A'}")
    ...
    waves = {
        "original": original,
        "impaired-listener-preview": impaired,
    ...
    signal_space_metrics = {
        "impaired": {
            "si_sdr": si_sdr(original, impaired).mean().item(),
            "spectral_distance": log_spectral_distance(original, impaired),
            "hf_recovery": high_frequency_energy_ratio(original, impaired, sr=sr),
            "intelligibility": intelligibility_proxy(original, impaired),
        },
        "baseline_model": {
            "si_sdr": si_sdr(original, baseline_enh).mean().item(),
            "spectral_distance": log_spectral_distance(original, baseline_enh),
            "band_energy": bandwise_energy(baseline_enh, sr=sr),
        },
        "calibration": {
            "si_sdr": si_sdr(original, calibration).mean().item(),
            "spectral_distance": log_spectral_distance(original, calibration),
            "hf_recovery": high_frequency_energy_ratio(original, calibration, sr=sr),
            "intelligibility": intelligibility_proxy(original, calibration),
            "gain": gain_stats(original, calibration),
            "clipping": clipping_stats(calibration),
        },
        "conditioned": {
            "si_sdr": si_sdr(original, conditioned_enh).mean().item(),
            "spectral_distance": log_spectral_distance(original, conditioned_enh),
            "hf_recovery": high_frequency_energy_ratio(original, conditioned_enh, sr=sr),
            "intelligibility": intelligibility_proxy(original, conditioned_enh),
            "band_energy": bandwise_energy(conditioned_enh, sr=sr),
            "gain": gain_stats(original, conditioned_enh),
            "clipping": clipping_stats(conditioned_enh),
        },
    }

    listener_space = {
        "impaired_input": listener_space_metrics(original, impaired, audiogram_tensor, sr=sr, output_already_impaired=True),
        "baseline_model": listener_space_metrics(original, baseline_enh, audiogram_tensor, sr=sr),
        "calibration": listener_space_metrics(original, calibration, audiogram_tensor, sr=sr),
        "conditioned": listener_space_metrics(original, conditioned_enh, audiogram_tensor, sr=sr),
    }

    comparison = {
        "question": "Does conditioned ML beat the calibration baseline?",
        "calibration_vs_conditioned_signal_space_delta": {
            "si_sdr": signal_space_metrics["conditioned"]["si_sdr"] - signal_space_metrics["calibration"]["si_sdr"],
            "spectral_distance": signal_space_metrics["conditioned"]["spectral_distance"]
            - signal_space_metrics["calibration"]["spectral_distance"],
            "hf_recovery": signal_space_metrics["conditioned"]["hf_recovery"] - signal_space_metrics["calibration"]["hf_recovery"],
        },
        "calibration_vs_conditioned_listener_space_delta": {
    ...
    logger.info("Calibration is the deterministic baseline; conditioned model is the challenger.")
