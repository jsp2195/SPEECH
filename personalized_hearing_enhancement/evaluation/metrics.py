...
def listener_space_metrics(clean: torch.Tensor, estimate: torch.Tensor, audiogram: torch.Tensor, sr: int) -> dict[str, float]:
    if clean.ndim == 1:
    ...
    heard_estimate = apply_hearing_loss(estimate, audiogram, sr=sr)
    return {
        "listener_si_sdr": float(si_sdr(heard_clean, heard_estimate).mean().item()),
        "listener_log_spectral_distance": log_spectral_distance(heard_clean, heard_estimate),
    }
