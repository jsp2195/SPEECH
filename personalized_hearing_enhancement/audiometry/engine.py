...
def _update_reliability(session: AudiometrySession, posterior_mean: float, posterior_std: float, amplitude_db_hl: float, heard: bool) -> None:
    ...
    strong_expect_heard = margin > max(3.0, 0.5 * posterior_std)
    strong_expect_not = margin < -max(3.0, 0.5 * posterior_std)
    ...
    session.reliability_score = float(max(0.0, 1.0 - 1.5 * inconsistency_ratio))
...
def run_hearing_test(
    ...
    *,
    ...
    if estimator_mode == "bayesian":
        ...
        joint_state = estimator.initialize_joint_state(session)
        session.joint_inference_state = joint_state.__dict__

        while not estimator.is_complete(joint_state):
            freq, amplitude, ig = estimator.select_next_stimulus(joint_state)
            session.active_frequency_index = session.frequencies_hz.index(freq)
            session.active_amplitude_db_hl = float(amplitude)

            if verbose:
                unfinished = sum(0 if joint_state.completed_by_freq[f] else 1 for f in session.frequencies_hz)
                print(
                    f"Trial {session.total_trials+1}: freq={freq}Hz amp={amplitude:.1f}dB HL "
                    f"mean={joint_state.posterior_mean_by_freq[freq]:.1f} std={math.sqrt(joint_state.posterior_variance_by_freq[freq]):.2f} "
                    f"unfinished={unfinished} eig={ig:.4f}"
                )

            if mode == "simulated":
                if response_callback is not None:
                    heard = bool(response_callback(freq, amplitude, joint_state.trial_count_by_freq[freq]))
                else:
                    gt = float(ground_truth_audiogram[session.frequencies_hz.index(freq)])
                    heard, _ = _simulated_heard(amplitude, gt, rng, responder_cfg, trial_idx=joint_state.trial_count_by_freq[freq] + 1)
            else:
                heard = _to_heard(input(f"Heard tone at {freq} Hz? [y/n]: "))

            _update_reliability(
                session,
                posterior_mean=joint_state.posterior_mean_by_freq[freq],
                posterior_std=math.sqrt(max(joint_state.posterior_variance_by_freq[freq], 0.0)),
                amplitude_db_hl=amplitude,
                heard=heard,
            )
            fs = session.state_for(freq)
            fs.trials.append(
                {
                    "trial": float(len(fs.trials) + 1),
                    "amplitude_db_hl": float(amplitude),
                    "heard": bool(heard),
                }
            )
            joint_state = estimator.update_joint_state(joint_state, frequency_hz=freq, amplitude_db_hl=amplitude, heard=heard)
            session.joint_inference_state = joint_state.__dict__
            estimator._sync_session_frequency_states(session, joint_state)

            if save_progress_path:
                Path(save_progress_path).write_text(json.dumps(session.to_dict(), indent=2), encoding="utf-8")

        summary = estimator.summarize_joint_state(joint_state)
        ...
        for idx, freq in enumerate(session.frequencies_hz):
            ...
            while not state.complete:
                if mode == "simulated":
                    ...
                    heard, _ = _simulated_heard(state.current_db_hl, gt, rng, responder_cfg, trial_idx=len(state.trials) + 1)
                else:
    ...
    metadata: dict[str, str | int | float | bool] = {
        ...
        "lapse_rate": cfg.lapse_rate,
        "guess_rate": cfg.guess_rate,
        "joint_latent_scaffold": estimator_mode == "bayesian",
    }
